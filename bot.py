"""
PAXG/USDT Telegram Signal Bot
-----------------------------
Price source: Coinbase public spot price endpoint (PAXG-USD, no API key required)
...
PAXG is a gold-backed token (1 PAXG ≈ 1 troy oz of gold), but unlike spot gold/XAU it
trades 24/7 on crypto markets — useful for tracking gold-like price action on weekends
when your Gold bot's forex market is closed.

Same structure as the current BTC, ETH, and ETC bots:
...
logger = logging.getLogger("paxg_bot")
...
# Config
# ---------------------------------------------------------------------------
COINBASE_URL = "https://api.coinbase.com/v2/prices/PAXG-USD/spot"
...
@app.route("/")
def home():
    return "PAXG/USDT bot is alive.", 200
...
# ---------------------------------------------------------------------------
def fetch_price():
    """Fetch the latest PAXG/USD spot price from Coinbase. Returns float or None.
...
async def status_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if price is not None:
        ...
        await update.message.reply_text(
            "⚠️ Couldn't fetch the current PAXG price. Try again shortly."
        )
...
async def price_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    price = fetch_price()
    if price is not None:
        await update.message.reply_text(f"🥇 PAXG/USDT: ${price:,.2f}")
    else:
        await update.message.reply_text("⚠️ Couldn't fetch the current PAXG price.")
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "refresh":
        ...
        if price is not None:
            ...
            await query.edit_message_text("⚠️ Couldn't fetch the current PAXG price.")
...
def main():
    ...
    logger.info("PAXG/USDT bot starting...")
