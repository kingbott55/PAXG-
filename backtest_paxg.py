...
backtest_paxg.py
----------------
Purpose: contains all trading intelligence and market analysis for the PAXG
bot. paxg_bot.py handles Telegram/scheduling/infra and calls analyze() here
...
logger = logging.getLogger("backtest_paxg")
...
def fetch_spot_price():
    """Latest PAXG/USD spot price from Coinbase. Returns float or None."""
    try:
...
def fetch_candles(granularity, limit=200):
    ...
    try:
        resp = requests.get(
            ...
            headers={"User-Agent": "paxg-analysis-engine/1.0"},
            timeout=15,
        )
        resp.raise_for_status()
        raw = resp.json()
...
# ---------------------------------------------------------------------------
# Final AI Decision Engine — the single entry point paxg_bot.py calls
# ---------------------------------------------------------------------------

def analyze():
    """
    Orchestrates every section above into one trading decision. Returns a
    dict ready for paxg_bot.py's message formatting, or None if live data
...
if __name__ == "__main__":
    # Quick manual smoke test: `python backtest_paxg.py`
