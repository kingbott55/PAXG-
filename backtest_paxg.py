"""
PAXG Bot Backtester (Full Logic + P&L Simulation)
------------------------------------------------------------------------
Replays the EXACT current paxg_bot.py signal pipeline against real historical
data:
...
HOW TO RUN:
  ...
  2. python backtest_paxg.py
  3. Results print to console, plus a full per-signal CSV
     (backtest_paxg_results.csv) including confidence AND per-trade P&L.
"""
...
# ---------------------------------------------------------------------------
# Config — mirrors paxg_bot.py exactly
# ---------------------------------------------------------------------------
RSI_PERIOD = 15
MA_SHORT = 5
MA_MED = 10
...
# ---------------------------------------------------------------------------
# Indicator logic — copied verbatim from paxg_bot.py
# ---------------------------------------------------------------------------
...
def run_backtest():
    ...
    for idx, sig, entry_price, confidence in signals:
        ...
        results_log.append(("PAXG/USDT bot (full logic)", idx, sig, round(entry_price, 4),
    ...
    print(f"\n=== PAXG/USDT bot — signal accuracy (direction only, no fees) ===")
    ...
    print(f"\n=== PAXG/USDT bot — P&L simulation (fees, slippage, position sizing) ===")
    ...
    if results_log:
        import csv
        with open("backtest_paxg_results.csv", "w", newline="") as f:
            writer = csv.writer(f)
        ...
        print("\nFull signal-by-signal log (including P&L per trade) saved to backtest_paxg_results.csv")
